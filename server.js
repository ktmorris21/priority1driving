...
import { validateSubmission, ValidationError } from './server/validation.js';
import { createCompletedPdf } from './server/pdf.js';
import { sendReviewEmail } from './server/email.js';
import { determineRecipients } from './config/routing.js';
...
app.post('/api/submit', async (req, res) => {
  ...
  try {
    ...
    const recipients = determineRecipients(data);
    ...
    res.status(500).json({
      ok: false,
      error: 'The review could not be submitted. Please try again or contact the administrator.',
      submissionId
    });
  }
});
